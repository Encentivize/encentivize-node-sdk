...
function makeRestRequest(options,callback){
    ...
    function tryClient(){
        function errorRetry(err){
            console.log("retrying encentivize sdk " + options.verb + ":" + url + ":" + retryCnt + ":" + JSON.stringify(err));
            if (retryCnt>0){
                retryCnt--;
                ...
                return callback('Error connecting to encentivize api:' + options.verb + ":" + url + ":" + JSON.stringify(err));
            }
        }
